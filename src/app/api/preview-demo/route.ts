...
const mockData = {
  "1d": {
    stats: {
      currentPrice: 1.84,
      currentPriceChange: 0.12,
      aum: 1023222,
      pastReturn: 2,
      periodText: "today"
    },
    chartData: baseChartData.slice(-1) // Just 1 day for simplicity in UI if needed
  },
  "7d": {
    stats: {
      currentPrice: 1.84,
      currentPriceChange: 0.45,
      aum: 1023222,
      pastReturn: 14,
      periodText: "this week"
    },
    chartData: baseChartData.slice(-7)
  },
  "30d": {
    stats: {
      currentPrice: 1.84,
    ...
    chartData: baseChartData.slice(-30)
  },
  "1y": {
    stats: {
      currentPrice: 1.84,
      currentPriceChange: 15.42,
      aum: 1023222,
      pastReturn: 184,
...
export async function GET(request: Request) {
  ...
  // Default to 30d if no valid period is provided
  const period = searchParams.get('period')?.toLowerCase() || '30d'; 
  
  const data = mockData[period as keyof typeof mockData] || mockData['30d'];
